...
export class CompraService {
    ...
    //Metodo para traer 1 dato
        async getDato(id:any){
            try{
                let dato = await this.prisma.compra.findFirst({
                    where: {id:id}
                });
        ...
        //editar
        async updateDatos(id:any, dto:CompraDto){
            try{
                //verifica si exite
                let existing = await this.prisma.compra.findFirst({
                    where: {id: id}
                });
